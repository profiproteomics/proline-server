...
class FragmentIonTable(peptide: Peptide,
  ...
  val fragments: HashMap[String, ArrayBuffer[Fragment]] = {
    ...
    for (aa <- aaSequence) {
      mass += FragmentIonTable.aminoAcids(aa).monoMass
      aa match {
    ...
    for (aa <- aaSequence.dropRight(1)) {
      val residue = FragmentIonTable.aminoAcids(aa)
      ...
      if (currentFragmentIonTypes.contains("a")) { 
        addSeries(_table, "a", currentFragmentIonTypes.getCharge("a"), new Fragment(bFragmentMz - FragmentIonTable.CO, position, nterPtmlNLMasse))
        addSeries(_table, "a*", currentFragmentIonTypes.getCharge("a"), new Fragment(bFragmentMz - FragmentIonTable.CO - FragmentIonTable.NH3, position, nterPtmlNLMasse))
        addSeries(_table, "a0", currentFragmentIonTypes.getCharge("a"), new Fragment(bFragmentMz - FragmentIonTable.CO - FragmentIonTable.H2O, position, nterPtmlNLMasse))
      }
      ...
      if (currentFragmentIonTypes.contains("b")) {
        ...
        addSeries(_table, "b*", currentFragmentIonTypes.getCharge("b"), new Fragment(if (nlResidues("NH3")(0) > 0) bFragmentMz - FragmentIonTable.NH3 else 0.0, position, nterPtmlNLMasse))
        addSeries(_table, "b0", currentFragmentIonTypes.getCharge("b"), new Fragment(if (nlResidues("H2O")(0) > 0) bFragmentMz - FragmentIonTable.H2O else 0.0, position, nterPtmlNLMasse))
      }
      
      if (currentFragmentIonTypes.contains("c")) addSeries(_table, "c", currentFragmentIonTypes.getCharge("c"), new Fragment(bFragmentMz + FragmentIonTable.NH3, position, nterPtmlNLMasse))

      if (currentFragmentIonTypes.contains("y")) {
        addSeries(_table, "y", currentFragmentIonTypes.getCharge("y"), new Fragment(yFragmentMz, reversePosition, cterPtmNLMasse))
        addSeries(_table, "y*", currentFragmentIonTypes.getCharge("y"), new Fragment(if (nlResidues("NH3")(1) > 0) yFragmentMz - FragmentIonTable.NH3 else 0.0, reversePosition, cterPtmNLMasse))
        addSeries(_table, "y0", currentFragmentIonTypes.getCharge("y"), new Fragment(if (nlResidues("H2O")(1) > 0) yFragmentMz - FragmentIonTable.H2O else 0.0, reversePosition, cterPtmNLMasse))
      }
  ...
  override def toString: String = FragmentIonTable.fragmentIonTableAsString(table)
