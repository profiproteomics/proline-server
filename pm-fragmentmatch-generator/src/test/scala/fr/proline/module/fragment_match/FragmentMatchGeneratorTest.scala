...
//      val proteinAccessionNumbers = Array("CH60_HUMAN")
      val msQueryId = 11
...
//      ptms += PTMFakeProvider.getPtmDefinition("Oxydation of M", 'M', PtmLocation.ANYWHERE).get
      ...
      //        new FragmentModificationMatch("deamidation of N and Q", true, 7)
      )
...
//      val fragmentMatches = new FragmentMatchManager(table.get, /*table.getNeutralLossTable,*/ spectrum)
...
//      val spectrumMatch = new SpectrumMatch(msQueryId, peptideMatchRank, table.get, fragmentMatches.get)
  ...
  def test2() {
    try {
      ...
      val spectrum = new Spectrum(
        ...
        peaklistId = 1)
      val peptideSequence = "APGFGDNR"
//      val proteinAccessionNumbers: Array[String] = Array("")
      val msQueryId = 11
      val peptideMatchRank = 1
//      var ptms: ArrayBuffer[PtmDefinition] = new ArrayBuffer[PtmDefinition]()
      // TODO add all the modifications (it should contain all the ptms, not just one !!)
