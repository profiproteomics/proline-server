...
class ExpiredMessageConsumer(queue: Queue, connection: Connection, serviceMonitoringNotifier: IServiceMonitoringNotifier) extends Runnable with LazyLogging {
      ...
      while (goOn) {
        ...
        try {
          ...
          if (jmsMessage == null) {
            ...
            val err: JSONRPC2Error = new JSONRPC2Error(JMSConstants.EXPIRED_MSG_ERROR_CODE, "Message has expired.");
            val jsonResponse: JSONRPC2Response = new JSONRPC2Response(err, jmsMessageId);

            val replyDestination = jmsMessage.getJMSReplyTo();
            val serviceName = jmsMessage.getStringProperty(JMSConstants.PROLINE_SERVICE_NAME_KEY);
            if (replyDestination == null) {
              logger.warn("Request JMS Message has no JMSReplyTo Destination : Cannot send JSON Response to Client");
            } else {

              /* Notify */
              val serviceEvent = new ServiceEvent(jsonResponse.getID.toString(), jmsMessageId, serviceName, ServiceEvent.EVENT_FAIL);
