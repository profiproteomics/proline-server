...
object MascotResultFile extends Logging {
  

  /**
   * Load MatrixScience Mascot Parser dll
   */
  private var _isMascotLibLoaded: Boolean = false
  def isMascotLibLoaded = _isMascotLibLoaded
  
  def initMascotLib() {
    if( ! isMascotLibLoaded ) {
      try {
        logger.debug("loading msparser dll" )
