...
abstract class AbstractPeptideMatchView extends AbstractProtSetToTypicalProtMatchView {
  ...
  override def buildRecord(buildingContext: IRecordBuildingContext): Map[String, Any] = {
    ...
    Map(
      ...
      fields.PROTEIN_SETS_COUNT.toString -> identDS.validProtSetIdSetByPepMatchId.get(pepMatch.id).map(_.size).getOrElse(0),
      fields.PROTEIN_MATCHES_COUNT.toString -> identDS.validProtMatchIdSetByPepMatchId.get(pepMatch.id).map(_.size).getOrElse(0),
