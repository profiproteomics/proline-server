    ...
    for (i <- 0 to (nbSheets - 1)) {
      ...
      s.id match {
        case ExportConfigConstant.SHEET_INFORMATION => {
          viewType = DatasetViewType.MSI_SEARCH_EXTENDED
        }
        case ExportConfigConstant.SHEET_IMPORT => {
          viewType = DatasetViewType.IMPORT_AND_VALIDATION_PROPS
        }
        case ExportConfigConstant.SHEET_PROTEIN_SETS => {
          viewType = DatasetViewType.PROT_SET_TO_TYPICAL_PROT_MATCH
        }
        case ExportConfigConstant.SHEET_BEST_PSM => {
          viewType = DatasetViewType.PROT_SET_TO_BEST_PEPTIDE_MATCH
        }
        case ExportConfigConstant.SHEET_PROTEIN_MATCH => {
          viewType = DatasetViewType.PROT_SET_TO_PROT_MATCH
        }
        case ExportConfigConstant.SHEET_ALL_PSM => {
          viewType = DatasetViewType.PROT_SET_TO_ALL_PEPTIDE_MATCHES
        }
        case ExportConfigConstant.SHEET_MASTER_QUANT_PEPTIDE_ION => {
          viewType = DatasetViewType.MASTER_QUANT_PEPTIDE_ION
        }
        case ExportConfigConstant.SHEET_STAT => {
          viewType = DatasetViewType.STATISTICS
        }
        case other => {
          // should not happen
        }
