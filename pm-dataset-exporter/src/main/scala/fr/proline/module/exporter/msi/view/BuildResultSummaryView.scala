...
object BuildResultSummaryView {
  ...
  private def _builders: Map[IViewTypeEnumeration#Value, MsiIdentDataSet => IDataView ] = Map( 
    ResultSummaryViewType.MSI_SEARCH_EXTENDED -> { ds: MsiIdentDataSet => new MsiSearchExtendedView(ds) },
    ResultSummaryViewType.IMPORT_AND_VALIDATION_PROPS -> { ds: MsiIdentDataSet => new ResultParseAndFiltersView(ds.resultSummary) },
    ResultSummaryViewType.STATISTICS -> { ds: MsiIdentDataSet => new StatisticsView(ds.resultSummary) },
    ResultSummaryViewType.PEP_SET_TO_PROT_MATCH -> { ds: MsiIdentDataSet => new PepSetToProtMatchView(ds) },
    ResultSummaryViewType.PROT_SET_TO_PROT_MATCH -> { ds: MsiIdentDataSet => new ProtSetToProtMatchView(ds) },
    ResultSummaryViewType.PROT_SET_TO_TYPICAL_PROT_MATCH -> { ds: MsiIdentDataSet => new ProtSetToTypicalProtMatchView(ds) },
    ResultSummaryViewType.PROT_SET_TO_BEST_PEPTIDE_MATCH -> { ds: MsiIdentDataSet => new ProtSetToBestPepMatchView(ds) },
    ResultSummaryViewType.PROT_SET_TO_ALL_PEPTIDE_MATCHES -> { ds: MsiIdentDataSet => new ProtSetToAllPepMatchesView(ds) },
    ResultSummaryViewType.TYPICAL_PROT_MATCH_TO_ALL_PEP_MATCHES -> { ds: MsiIdentDataSet => new TypicalProtMatchToAllPepMatchesView(ds) }    
  )
