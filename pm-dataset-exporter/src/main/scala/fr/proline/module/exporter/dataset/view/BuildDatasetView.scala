	...
	private def _builders(exportConfig: ExportConfig): Map[IViewTypeEnumeration#Value, IdentDataSet => IDataView] = {
			...
			for (sheet <- exportConfig.sheets) {
				...
				case ExportConfigConstant.SHEET_IMPORT => {
					buildMap += (DatasetViewTypes.IMPORT_AND_VALIDATION_PROPS -> { ds: IdentDataSet => new ImportAndValidationPropsView(ds, sheet, dateFormat, decimalFormat) })
				}
				case ExportConfigConstant.SHEET_PROTEIN_SETS => {
					buildMap += (DatasetViewTypes.PROT_SET_TO_TYPICAL_PROT_MATCH -> { ds: IdentDataSet => new ProtSetToTypicalProtMatchView(ds, sheet, dateFormat, decimalFormat, exportAllProteinSet) })
				}
				case ExportConfigConstant.SHEET_BEST_PSM => {
					buildMap += (DatasetViewTypes.PROT_SET_TO_BEST_PEPTIDE_MATCH -> { ds: IdentDataSet => new ProtSetToBestPepMatchView(ds, sheet, dateFormat, decimalFormat, exportAllProteinSet) })
				}
				case ExportConfigConstant.SHEET_PROTEIN_MATCH => {
					buildMap += (DatasetViewTypes.PROT_SET_TO_PROT_MATCH -> { ds: IdentDataSet => new ProtSetToProtMatchView(ds, sheet, dateFormat, decimalFormat, exportAllProteinSet) })
				}
				case ExportConfigConstant.SHEET_ALL_PSM => {
					buildMap += (DatasetViewTypes.PROT_SET_TO_ALL_PEPTIDE_MATCHES -> { ds: IdentDataSet => new ProtSetToAllPepMatchesView(ds, sheet, dateFormat, decimalFormat, exportAllProteinSet) })
				}
