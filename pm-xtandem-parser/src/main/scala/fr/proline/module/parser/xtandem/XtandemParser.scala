...
class XtandemParser(val xtandemFile: File,
    val parserContext: ProviderDecoratedExecutionContext) extends DefaultHandler with IResultFile with LazyLogging {
  ...
  def getResultSet(wantDecoy: Boolean): ResultSet = {
    ...
    // XTandem use the following PTMs as defaut variable PTMs
    // For reading facilities, we create this new variable. Mass values are XTandem values( and not Proline values ).
...
//      averageMass = 0.0)
      ...
      for (note <- gp.noteList) {
        ...
        if (dbGroupParametersNoteLabel.equals("output, sort results by")) {
          if (dbGroupParametersNoteInfo.equals("protein") || dbGroupParametersNoteInfo.isEmpty) {
            ...
            // TODO make sure that we quit properly if this option is activated
        ...
        } else if ((dbGroupParametersNoteLabel.equals("protein, cleavage semi") || dbGroupParametersNoteLabel.equals("refine, cleavage semi")) && dbGroupParametersNoteInfo.equals("yes")) {
          // TODO ? In Xtandem's xml results file, we can't distingue if semi cleavage is after standard research or refine research
          isSemiSpecific = true
        ...
        } else if (dbGroupParametersNoteLabel.equals("process, start time")) {
          // date is like "2015:04:27:15:41:28"
          val items = dbGroupParametersNoteInfo.split(":")
          searchStartTime = items(0)+"/"+items(1)+"/"+items(2)+" "+items(3)+":"+items(4)+":"+items(5)
        } else if (dbGroupParametersNoteLabel.equals("modelling, total proteins used")) {
          totalProteinUsed = dbGroupParametersNoteInfo.toInt
        }
      }
    ...
    fixedPtms.foreach(ptms => {
      ...
      if (_ptm.get != null) {
        fixedPtmDefs.append(_ptm.get); logger.warn("fixedPtmDefs : mono mass = " + ptms._1 + ", residue = " + ptms._2 + ", location = " + ptms._3 + " ... is " + _ptm.get.names.shortName)
      } else {
        logger.warn("Can not identify ptm with : mono mass = " + ptms._1 + ", residue = " + ptms._2 + ", location = " + ptms._3)
      }
    })
    ...
    variablePtms.foreach(ptms => {
      val _ptm = ptmProvider.getPtmDefinition(ptms._1, ptmMonoMassMargin, ptms._2, ptms._3)
      if (_ptm.get != null) {
        variablePtmDefs.append(_ptm.get); logger.warn("variablePtmDefs : mono mass = " + ptms._1 + ", residue = " + ptms._2 + ", location = " + ptms._3 + " ... is " + _ptm.get.names.shortName)
      } else {
        logger.warn("Can not identify ptm with : mono mass = " + ptms._1 + ", residue = " + ptms._2 + ", location = " + ptms._3)
      }
    })
    ...
    for (gm <- resultBioml.groupModelList) {
      ...
      for (gs <- gm.groupSupportList) {
        ...
        // experimental MoZ = (MH + (Z-1) * (PROTON_MASS)) / Z
        // Z-1 because mh already has 1H added
        val expMoz = (dGroupModelMh + (dbGroupModelZ-1) * MolecularConstants.PROTON_MASS) / dbGroupModelZ
//        val expMoz = dGroupModelMh/dbGroupModelZ
        ...
        for (gamlTrace <- gs.gamlTraceList) {
          ...
          if (dbGAMLXdataUnits.equals("MASSTOCHARGERATIO")) {
            ...
            val retentionTime: Float = if (dGroupModelRt.isDefined) dGroupModelRt.get.toFloat / 60 else 0.0F
        ...
        for (p <- gs.proteinList) {
          ...
          for (dbPeptideDomain <- dbProteinPeptide.domainList) {
            ...
            seqMatch = new SequenceMatch(
              ...
              residueBefore = if (dbDomainStart == 1) '-' else dbDomainPre.charAt(dbDomainPre.length() - 1), // X!Tandem gives up to 4 previous AA
              residueAfter = if (dbDomainEnd == dbProteinPeptideEnd) '-' else dbDomainPost.charAt(0), // X!Tandem gives up to 4 next AA
              isDecoy = false)
            ...
            if (!dbDomainSeqList.contains(dbDomainSeq)) { // Creating new peptide only if there is a new sequence, otherwise pgAdmin return duplicated sequence value
              // calculated mass = MH - PROTON_MASS
              peptide = new Peptide(
                sequence = dbDomainSeq,
                ptms = locatedPtms.toArray,
                calculatedMass = dbDomainMh - MolecularConstants.PROTON_MASS)
//                calculatedMass = dbDomainMh)
    ...
    msiSearchForResultSet = new MSISearch(
      ...
      userName = "",
      userEmail = "",
      searchedSequencesCount = totalProteinUsed,
      queriesCount = resultBioml.groupModelList.length)

    // return the ResultSet new object
    new ResultSet(
